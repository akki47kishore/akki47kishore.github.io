...
function validate(name, date, itemQuantitiesSet) {
    ...
    for (let key in itemQuantitiesSet) {
...
function showDetails(...args) {
    ...
    for (let item in data) {
        ...
        for (let value in data[item]) {
            if (typeof (data[item][value]) != "object") {
                ...
                for (let subCategoryValue in data[item][value]) {
                    if (data[item][value][subCategoryValue] != 0) {
                        subcategorySum = subcategorySum + data[item][value][subCategoryValue];
    ...
    $(`#${table} tbody`).on('click', 'tr', function () {
        let child = detailsTable.row(this).child;
        if (child.isShown()) {
            child.hide();
        } else {
            child.show();
